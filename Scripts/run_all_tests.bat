...
echo 80504004
echo fc000000
for /L %%i in (9,1,65536) do echo 00000000
) > rom_init.mem
iverilog -o test_cpu_core_mem.vvp SystemTests/tb_cpu_core_mem.v SystemTests/system_top.v %MEM_CORE% %PIPE_CORE% %ALU_CORE% %REG_CORE% %PERI_CORE%
if %errorlevel% neq 0 (echo Compilation Failed! && exit /b 1)
...
echo 80604010
echo fc000000
for /L %%i in (13,1,65536) do echo 00000000
) > rom_init.mem
iverilog -o test_cpu_core_cache.vvp SystemTests/tb_cpu_core_cache.v SystemTests/system_cache_top.v %MEM_CORE% %PIPE_CORE% %ALU_CORE% %REG_CORE% %PERI_CORE%
if %errorlevel% neq 0 (echo Compilation Failed! && exit /b 1)
